    ...
    async def ask_patrick(self, message: discord.Message | discord.Interaction, question: str):
        ...
        if number == 1:
            ...
            async with message.channel.typing():
                ...
                await ask_reply(message, "# **One day you will have to answer for your actions. And god.. may not be so merciful..**", send_in_channel=True)
        # Yes 450%%
        elif number <= 451:
        ...
        # Random bouncerline 50%%
            ...
            await ask_reply(message, process_fact(self.cm.sayings.get_line(None), self.cm.facts_manager, message, self.bot))
        # Drunk too much, random bouncerwords 49%%
        elif number <= 1000:
            await ask_reply(message, "*He's drank too many beers.*")
            await ask_reply(message, " ".join([_r.choice(self.cm.sayings.get_sayings_words()) for _ in range(_r.randint(1, 20))]), send_in_channel=True)
