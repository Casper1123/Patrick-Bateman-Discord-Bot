...
class GlobalFactAdminCog(CustomGroupCog, group_name='gfact'):
    ...
    async def _gfact_index_autocomplete_impl(self, _: Interaction, current: str) -> list[Choice[int]]:
        ...
        return [
            Choice[int](name=f'{lower + offset + 1}: {fact.text[:80]}', value=lower + offset + 1)
...
class GlobalAdminCog(CustomGroupCog, group_name='global'):
    ...
    async def ban_user(self, interaction: Interaction, user_id: int, reason: str | None = None,
        ...
        state: bool = self.mod.toggle_user_ban(user_id, interaction.user.id, reason)
    ...
    async def ban_guild(self, interaction: Interaction, guild_id: int, reason: str | None = None,
                        ephemeral: bool = False) -> None:
        state: bool = self.mod.toggle_guild_ban(guild_id, interaction.user.id, reason)
