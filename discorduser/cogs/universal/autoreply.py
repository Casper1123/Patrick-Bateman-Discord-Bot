...
class _AliasGlobalAdminCog(CustomGroupCog, group_name='alias'):
    ...
    async def edit_alias(self, interaction: Interaction, alias: str, new_name: str | None = None,
                         rate: int | None = None, ephemeral: bool = False):
    ...
    @app_commands.describe(json='Export data in JSON format.',
                           include_components='Include data on attached replies and triggers too.',
                           ephemeral=CFG.EPHEMERAL_DESCRIPTION)
    async def index(self, interaction: Interaction, json: bool = False, include_components: bool = False,
                    ephemeral: bool = True):
        aliases: list[
            SimpleAliasData] = self.repl.get_aliases()  # todo: expand for full data compat --> Author id added to it.
        if json:
            out: list[dict] = []
            for a in aliases:
                val = a.as_json()
    ...
    async def edit_trigger(self, interaction: Interaction, alias: str, index: int, text: str = None, rate: int = None,
        ...
        if text is None and rate is None:
            await self.client.user_feedback(interaction, title='Trigger edit failed',
                                            desc='You need to update at least one of text and rate.',
                                            ephemeral=ephemeral)
            return
        if rate is not None and not (1 <= rate <= 256):
            await self.client.user_feedback(interaction, title='Trigger edit failed',
...
class _ReplyGlobalAdminCog(CustomGroupCog, group_name='reply'):
    ...
    async def create_reply(self, interaction: Interaction, alias: str, reply_type: reply_types, text: str,
                           weight: int = 1, ephemeral: bool = False):
        if weight is not None and not 1 <= weight <= CFG.REPLY_WEIGHT_UPPER_BOUND:
            await self.client.user_feedback(interaction, title='Reply creation failed',
                                            desc=f'Weight not in range [1..{CFG.REPLY_WEIGHT_UPPER_BOUND}].',
                                            ephemeral=ephemeral)
        if reply_type == 'text':
    ...
    async def edit_reply(self, interaction: Interaction, alias: str, index: int, text: str = None, weight: int = None,
        ...
        if text is None and weight is None:
            await self.client.user_feedback(interaction, title='Reply edit failed',
                                            desc='You need to update at least one of text and weight.')
            return
        ...
        try:
            ...
            await self.client.user_feedback(interaction, title='Reply edit failed', desc='Reply index out of bounds',
                                            ephemeral=ephemeral)
            return
    ...
    async def delete_reply(self, interaction: Interaction, alias: str, index: int, ephemeral: bool = False):
        try:
            ...
            await self.client.user_feedback(interaction, title='Reply deletion failed',
                                            desc='Reply index out of bounds',
                                            ephemeral=ephemeral)
            return
