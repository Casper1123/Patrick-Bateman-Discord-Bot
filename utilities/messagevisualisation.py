...
def _extract_embedded_url(message_content: str) -> str | None:
    markdown_link_regex = _re.compile(r"\[.*?]\((https?://[^\s)]+)\)")  # Matches [text](link)
    url_regex = _re.compile(r"^(https?://\S+)$")  # Matches standalone URLs
    file_extension_regex = _re.compile(r"(\.png|\.jpg|\.jpeg|\.gif|\.webp|\.mp4|\.webm|\.mov)(\?.*)?$")
